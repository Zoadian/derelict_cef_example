...
extern(Windows) {
	void on_before_command_line_processing(cef_app_t* self, const(cef_string_t)* process_type, cef_command_line_t* command_line) nothrow @nogc {
		// DEBUG_CALLBACK("on_before_command_line_processing\n");
	}

	void on_register_custom_schemes(cef_app_t* self, cef_scheme_registrar_t* registrar) nothrow @nogc {
		// DEBUG_CALLBACK("on_register_custom_schemes\n");
	}

	cef_resource_bundle_handler_t* get_resource_bundle_handler(cef_app_t* self) nothrow @nogc {
		// DEBUG_CALLBACK("get_resource_bundle_handler\n");
		return null;
	}

	cef_browser_process_handler_t* get_browser_process_handler(cef_app_t* self) nothrow @nogc {
		// DEBUG_CALLBACK("get_browser_process_handler\n");
		return null;
	}

	cef_render_process_handler_t* get_render_process_handler(cef_app_t* self) nothrow @nogc {
